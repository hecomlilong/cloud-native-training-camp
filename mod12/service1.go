...
func (p *Service1Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if w == nil {
		utils.GlogWrapper.Warning("Service1Handler.ServeHTTP got nil ResponseWriter")
		return
	}
	if r == nil {
		utils.GlogWrapper.Warning("Service1Handler.ServeHTTP got nil Request")
	}
	p.HeaderHandler(w, r)
	...
	p.ForwardHandler(w, r, "service2")
}

// 将header内存透传到下一个微服务
func (p *Service1Handler) ForwardHandler(w http.ResponseWriter, r *http.Request, serviceName string) {
	err := utils.ForwardHandler(w, r, serviceName)
	if err != nil {
		utils.GlogWrapper.Warningf("Service1Handler.ForwardHandler:client.Do:err:%v", err)
		return
	}
}

// 接收客户端 request，并将 request 中带的 header 写入 response header
func (p *Service1Handler) HeaderHandler(w http.ResponseWriter, r *http.Request) {
...
func (p *Service1Handler) VersionHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Add("Version", os.Getenv("VERSION"))
}

// Server 端记录访问日志包括客户端 IP，HTTP 返回码，输出到 server 端的标准输出
func (p *Service1Handler) StdoutHandler(w http.ResponseWriter, r *http.Request, statusCode int) {
	utils.GlogWrapper.Infof("client ip address:%s", utils.FindClientIp(r))
	utils.GlogWrapper.Infof("http code:%d", statusCode)
}

// 当访问 localhost/healthz 时，应返回 200
func (p *Service1Handler) HealthzHandler(w http.ResponseWriter, r *http.Request) (statusCode int) {
...
func (p *Service1Handler) MatricsHandler(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path == "/metrics" {
		promhttp.Handler()
	}
}

func (p *Service1Handler) RootHandler(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path != "/hello" {
		return
	}
